-- Migration: Create end_production_status_atomic_v2
-- Part of: Production System Refactor - Chunk 1 (Database Foundation)
-- Purpose: Same as end_production_status_atomic but calls v6 (no consumption)

CREATE OR REPLACE FUNCTION end_production_status_atomic_v2(
  p_session_id UUID,
...
BEGIN
  ...
  IF v_session.job_item_id IS NOT NULL AND v_session.job_item_step_id IS NOT NULL THEN
    v_wip_result := update_session_quantities_v6(p_session_id, p_quantity_good, p_quantity_scrap);

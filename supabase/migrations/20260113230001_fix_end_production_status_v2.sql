-- Migration: Update end_production_status_atomic to v2
-- Part of: Job System Overhaul (Phase 4)
-- Purpose:
--   1. Fix column reference: job_item_station_id -> job_item_step_id
--   2. Use update_session_quantities_atomic_v3 instead of v2
--   3. Record job_item_id and job_item_step_id on the status event
...
BEGIN
  ...
  --    This records which job item/step was being worked during this production period
  ...
  -- 7. If session has job_item_id, update WIP balances using v3 function
  --    (v3 uses the renamed job_item_step_id column)
...
v2: Records job_item_id and job_item_step_id on the status event, uses update_session_quantities_atomic_v3.';

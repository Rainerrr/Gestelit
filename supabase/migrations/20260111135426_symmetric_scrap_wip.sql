-- Migration: Make scrap WIP tracking symmetric with good tracking
--
-- FIXES:
-- 1. Scrap decrease now properly tracks originated vs pulled scrap
-- 2. Originated scrap doesn't return to upstream (same as good)
-- 3. Pulled scrap returns to upstream via LIFO (same as good)
-- 4. View now separates good vs scrap accounting correctly
--
-- Previous behavior (BUG):
--   Scrap decrease only returned "pulled" scrap, but had no concept of
--   "originated" scrap. If you reported 10 scrap but only 5 came from
--   upstream, the other 5 would just vanish on decrease with no validation.
--
-- New behavior (SYMMETRIC):
--   Same as good: originated_scrap = total_scrap - pulled_scrap
--   On decrease, originated scrap is consumed first (no return),
--   then pulled scrap is returned via LIFO.
...
BEGIN
  ...
  ELSIF v_delta_good < 0 THEN
    ...
    -- LIFO: first consume originated (they don't return anywhere)
  ...
  -- SYMMETRIC with good decrease:
  -- - Originated scrap: just removed (no return to upstream)
  -- - Pulled scrap: returned to upstream via LIFO reversal
  -- ============================================
  ELSIF v_delta_scrap < 0 THEN
    ...
    -- Calculate originated scrap (SYMMETRIC with good)
    -- originated_scrap = total_scrap - pulled_scrap
    ...
    -- If we need to reverse pulled scrap, do LIFO (newest first)
    IF v_pulled_reduce > 0 THEN
      FOR v_consumption IN
        SELECT wc.*, jis.id as upstream_jis_id
        FROM public.wip_consumptions wc
      ...
      LOOP
        ...
        -- Return to upstream balance (was erroneously consumed as scrap)
        UPDATE public.wip_balances
        SET good_available = good_available + v_return_amount
        WHERE job_item_station_id = v_consumption.upstream_jis_id
          AND job_item_id = v_session.job_item_id;
...
-- Documentation
COMMENT ON FUNCTION public.update_session_quantities_atomic_v2 IS 'Atomically updates session quantities with WIP balance management. Good products flow downstream. Scrap consumes from upstream but does not pass to downstream. Both good and scrap track originated vs pulled for symmetric decrease behavior. Returns error_code WIP_DOWNSTREAM_CONSUMED if good reduction blocked.';

-- ============================================
-- Part 2: Fix the accounting view
-- Separates good vs scrap accounting correctly
-- ============================================
...
SELECT
  ...
  (COALESCE(s.total_good, 0) - COALESCE(good_pulls.pulled, 0))::INTEGER AS originated_good,
  -- Scrap accounting (NEW - symmetric with good)
  COALESCE(s.total_scrap, 0) AS total_scrap,
  COALESCE(scrap_pulls.pulled, 0)::INTEGER AS pulled_scrap,
  (COALESCE(s.total_scrap, 0) - COALESCE(scrap_pulls.pulled, 0))::INTEGER AS originated_scrap
FROM public.sessions s
LEFT JOIN (
  SELECT consuming_session_id, SUM(good_used) AS pulled
  FROM public.wip_consumptions

-- Migration: Extend rebuild_job_item_steps() to handle 'pipeline' kind
-- Part of: Job System Overhaul (Phase 2)
-- Purpose: Add support for pipeline_preset_id in job item creation
--
-- This extends the existing RPC to handle kind='pipeline' by looking up
-- station IDs from the pipeline_preset_steps table.
...
BEGIN
  ...
  ELSIF v_job_item.kind = 'pipeline' THEN
    ...
    WHERE preset_id = v_job_item.pipeline_preset_id;
    ...
    WHERE pps.preset_id = v_job_item.pipeline_preset_id

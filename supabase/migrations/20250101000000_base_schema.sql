-- Base schema bootstrap to support local Supabase CLI runs
...
do $$
begin
  if not exists (select 1 from pg_type where typname = 'checklist_kind') then
...
do $$
begin
  if not exists (select 1 from pg_type where typname = 'reason_type') then
...
do $$
begin
  if not exists (select 1 from pg_type where typname = 'status_event_state') then
